...
function readViewportMode() {
  ...
  const phoneUa = PHONE_UA.test(String(navigator.userAgent || ""));
  const coarse = window.matchMedia?.("(pointer: coarse)")?.matches;
  const narrow = width < 768;
  const portraitPhone = (phoneUa || coarse) && width < 900 && height >= width;
  return narrow || portraitPhone ? "portrait" : "landscape";
}

function useViewportMode() {
  const [mode, setMode] = useState(readViewportMode);

  useEffect(() => {
    let frame = 0;
    const update = () => {
      window.cancelAnimationFrame(frame);
      frame = window.requestAnimationFrame(() => setMode(readViewportMode()));
    };
...
export default function CustomerDashboardResponsive() {
  ...
  const phoneMode = viewportMode === "portrait";
    ...
    <div className="min-h-dvh bg-[#020617] px-3 pb-24 pt-3 text-slate-100" data-sw-layout="portrait">
      ...
      {dayTradingEnabled ? (
        <button type="button" onClick={() => nav("/customer/day-trading-futures")} className="mt-3 flex w-full items-center gap-3 rounded-[1.2rem] border border-emerald-400/25 bg-emerald-500/[.07] p-3 text-left shadow-[0_0_28px_rgba(16,185,129,.08)]">
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-emerald-400/25 bg-emerald-500/10 text-emerald-200"><Activity className="h-4 w-4" /></span>
          <span className="min-w-0 flex-1"><span className="block text-[11px] font-black text-white">Day Trade</span><span className="mt-0.5 block text-[9px] leading-4 text-slate-500">MNQ futures signal dashboard · analysis mode</span></span>
      ...
      <nav className="fixed inset-x-3 bottom-[max(.45rem,env(safe-area-inset-bottom))] z-[150] grid grid-cols-5 items-end rounded-[1.3rem] border border-white/10 bg-[#030817]/95 px-2 py-2 shadow-[0_-10px_50px_rgba(2,6,23,.8)] backdrop-blur-2xl">
