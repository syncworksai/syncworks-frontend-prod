...
export default function PMPropertyCreate() {
  ...
  async function saveProperty(event) {
    ...
    try {
      ...
      navigate(response.data?.id ? `/pm/properties/${response.data.id}` : "/pm/properties", { replace: true });
    } catch (caught) {
    ...
    <main className="relative z-10 mx-auto max-w-3xl px-4 pb-[calc(13rem+env(safe-area-inset-bottom))] pt-6">
      <form onSubmit={saveProperty} className="space-y-5">
        <section className="rounded-[28px] border border-blue-500/20 bg-[#07111f]/95 p-5 sm:p-6">
          <div className="mb-5"><div className="text-[10px] font-semibold uppercase tracking-[0.22em] text-cyan-300">Portfolio record</div><h1 className="mt-2 text-2xl font-semibold tracking-tight">Property information</h1><p className="mt-2 text-sm leading-6 text-slate-400">Create the property record now. Units, tenants, leases, and maintenance will connect to this property.</p></div>
          {error ? <div className="mb-5 rounded-2xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm leading-5 text-rose-100">{error}{!workspace?.id ? <button type="button" onClick={() => navigate("/pm/settings")} className="mt-3 block rounded-xl border border-rose-300/30 px-3 py-2 text-xs font-bold">Open Portfolio Settings</button> : null}</div> : null}
        ...
        <div className="sticky bottom-[calc(6.5rem+env(safe-area-inset-bottom))] z-30 rounded-[26px] border border-cyan-400/25 bg-[#07111f]/95 p-3 shadow-[0_18px_60px_rgba(0,0,0,0.55)] backdrop-blur-xl sm:static sm:flex sm:justify-end"><button type="submit" disabled={!canSave || saving || loadingWorkspace} className="min-h-14 w-full rounded-2xl bg-gradient-to-r from-cyan-400 to-blue-500 px-5 text-sm font-bold text-slate-950 disabled:cursor-not-allowed disabled:opacity-45 sm:w-auto sm:min-w-48">{saving ? "Saving property..." : loadingWorkspace ? "Loading portfolio..." : "Save property"}</button></div>
      </form>
    </main>
  </div>;
}
