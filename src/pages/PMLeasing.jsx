...
export default function PMLeasing() {
  ...
  return <div className="min-h-screen bg-transparent text-slate-100"><main className="space-y-5 px-4 py-6 sm:px-6">
    ...
    {tab === "pipeline" ? <section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/90 p-4 sm:p-5"><div className="space-y-3">{prospects.length ? prospects.map((item) => <article key={item.id} className="rounded-3xl border border-slate-700/70 bg-black/25 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-lg font-black text-white">{item.full_name}</div><div className="mt-1 text-xs text-slate-400">{item.email} · {item.phone || "No phone"}</div><div className="mt-2 text-xs text-slate-500">{item.assigned_property_name ? `${item.assigned_property_name} · ${item.assigned_unit_name}` : "No unit matched"}{item.section8_requested ? " · Section 8 requested" : ""}</div></div><select className={inputClass} style={{width:220}} value={item.stage} onChange={(e) => updateProspect(item,{stage:e.target.value})}>{["LEAD","APPLICATION_SENT","APPLICATION_RECEIVED","SCREENING","APPROVED","SHOWING_SCHEDULED","READY_FOR_ONBOARDING","CONVERTED","DECLINED","WITHDRAWN"].map((s)=><option key={s} value={s}>{s.replaceAll("_"," ")}</option>)}</select></div><div className="mt-3 flex flex-wrap gap-2">{["APPROVED","SHOWING_SCHEDULED","READY_FOR_ONBOARDING"].includes(item.stage) ? <Button tone="cyan" onClick={() => convert(item)}>Convert to Tenant</Button> : null}</div></article>) : <div className="py-12 text-center text-sm text-slate-500">No prospects yet.</div>}</div></section> : null}
