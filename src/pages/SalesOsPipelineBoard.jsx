    ...
    <CardShell>
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 flex items-center gap-2">
          ...
          <IconBtn title="Delete prospect" tone="rose" disabled={savingId === p.id} onClick={() => onDelete(p)}>
            {savingId === p.id ? "…" : "🗑️"}
          </IconBtn>
        </div>
      </div>
...
export default function SalesOsPipelineBoard() {
  ...
  // ✅ New Prospect bubble modal
  const [npModal, setNpModal] = useState({ open: false });
  ...
  const filteredProspects = useMemo(() => {
    ...
    let out = list;
    if (agentFilter !== "all") out = out.filter((p) => String(p.assigned_member_id || "") === String(agentFilter));
    if (statusFilter !== "ALL") out = out.filter((p) => String(p.stage_id || "") === String(statusFilter));
    return out;
  ...
  /** ✅ Board owns pipeline selection */
  function goToPipeline(pid) {
    if (!pid) return;
    nav(`/sales/board?pipeline_id=${encodeURIComponent(pid)}`);
  }

  /** ✅ Create pipeline here (Board owns it) */
  async function createPipelineHere() {
    const name = prompt("Sales Pipeline name:");
    if (!name) return;

    try {
      const res = await api.post("/sales/pipelines/", { name: name.trim() });
      const id = String(res.data?.id || "");
      await loadPipelines();
      if (id) nav(`/sales/board?pipeline_id=${encodeURIComponent(id)}`);
    } catch (e) {
      alert(e?.response?.data?.detail || "Failed to create pipeline.");
    }
  }

  /** ✅ Open New Prospect bubble (always available; requires pipelineId) */
  function openNewProspect() {
    if (!pipelineId) return alert("Pick a pipeline first.");
    ...
    setNpForm((s) => ({
      ...
      phone: "",
      notes: "",
      stage_id: String(firstStage?.id || s.stage_id || ""),
      secondary_status: "",
    }));
    setNpFiles([]);
    setNpModal({ open: true });
  }

  async function createProspectFromModal() {
    if (!pipelineId) return alert("Missing pipeline_id.");
    if (!npForm.full_name.trim()) return alert("Name is required.");
    if (!npForm.stage_id) return alert("Pick a Status.");

    setNpBusy(true);
    try {
      await api.post("/sales/prospects/", {
        pipeline_id: Number(pipelineId),
        stage_id: Number(npForm.stage_id),
        full_name: npForm.full_name.trim(),
        email: npForm.email.trim(),
        phone: npForm.phone.trim(),
        notes: npForm.notes || "",
        secondary_status: (npForm.secondary_status || "").trim(),
      });

      setNpModal({ open: false });
      await load();
    } catch (e) {
  ...
  async function importCsv() {
    ...
    try {
      ...
      if (statusFilter !== "ALL") fd.append("default_stage_id", String(statusFilter));
  ...
  const middleActions = (
    <>
      <button
        ...
        disabled={!pipelineId}
        title={!pipelineId ? "Pick a pipeline first" : "Create prospect"}
      >
        + New Prospect
      </button>
      ...
      <button
        ...
        className={cx(
          ...
          !pipelineId
            ? "bg-slate-900/40 border-slate-800 text-slate-500 cursor-not-allowed"
            : "bg-emerald-500/10 border-emerald-500/25 hover:bg-emerald-500/15 text-emerald-200"
        )}
        disabled={!pipelineId}
        title={!pipelineId ? "Pick a pipeline first" : "Upload/import CSV"}
      >
        ⬆️ Upload / Import CSV
      </button>
      ...
      <button
        type="button"
        onClick={createPipelineHere}
        className="text-xs rounded-2xl px-3 py-2 bg-indigo-500/15 border border-indigo-500/30 hover:bg-indigo-500/20 text-indigo-200"
        title="Create Sales Pipeline ($9.95/mo after upgrade)"
      >
        + Create Pipeline
      </button>
      ...
      <button
        type="button"
        onClick={pipelineId ? load : loadPipelines}
      ...
      >
        {loading || pipelinesLoading ? "Loading..." : "Refresh"}
      </button>
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100">
      <ModeBar title="Sales OS • Board" subtitle={pipelineId ? `Pipeline #${pipelineId}` : "Pick or create a pipeline"} />

      {/* ✅ Middle Sales OS nav + actions */}
      <SalesOsSubNav pipelineId={pipelineId} active="board" actions={middleActions} />

      <div className="max-w-6xl mx-auto px-4 pb-16">
        {!pipelineId ? (
          <div className="mt-6 rounded-3xl border border-amber-500/30 bg-amber-500/10 p-5 text-amber-100">
            <div className="font-semibold">Pick a pipeline</div>
            <div className="text-xs text-amber-100/80 mt-1">Board owns pipeline selection.</div>

            <div className="mt-4 flex items-center gap-2 flex-wrap">
              <select
                className="bg-slate-950/60 border border-slate-800 rounded-2xl px-3 py-2 text-sm min-w-[280px]"
                defaultValue=""
                onChange={(e) => goToPipeline(e.target.value)}
              ...
              <button
                type="button"
                onClick={() => nav("/upgrade")}
                className="text-xs rounded-2xl px-3 py-2 bg-fuchsia-500/12 border border-fuchsia-500/30 hover:bg-fuchsia-500/16 text-fuchsia-200"
                title="Upgrade Sales OS ($9.95/mo)"
              >
                Upgrade Sales OS
              </button>
            </div>
        ...
        {err ? <div className="mt-6 text-sm text-red-300 bg-red-900/20 border border-red-800 rounded-2xl p-3">{err}</div> : null}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-2xl rounded-3xl border border-slate-800 bg-slate-950 p-5">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="text-lg font-extrabold">New Prospect</div>
                <div className="text-xs text-slate-400 mt-1">
                  Pipeline <span className="text-slate-200">#{pipelineId}</span>
                </div>
              </div>
              <button type="button" onClick={() => setNpModal({ open: false })} className="text-slate-300 hover:text-white">
                ✕
              </button>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
              <input
                ...
                onChange={(e) => setNpForm((s) => ({ ...s, full_name: e.target.value }))}
              />
              <input
                className="bg-slate-950/60 border border-slate-800 rounded-2xl px-3 py-2"
                placeholder="Email"
                value={npForm.email}
                onChange={(e) => setNpForm((s) => ({ ...s, email: e.target.value }))}
              />
              <input
                className="bg-slate-950/60 border border-slate-800 rounded-2xl px-3 py-2"
                placeholder="Phone"
                value={npForm.phone}
                onChange={(e) => setNpForm((s) => ({ ...s, phone: e.target.value }))}
              />

              <select
                className="bg-slate-950/60 border border-slate-800 rounded-2xl px-3 py-2"
                value={npForm.stage_id}
                onChange={(e) => setNpForm((s) => ({ ...s, stage_id: e.target.value }))}
              >
              ...
              <input
                className="md:col-span-2 bg-slate-950/60 border border-slate-800 rounded-2xl px-3 py-2"
                placeholder="Secondary Tag Status (optional) — e.g. Hot / Warm / Follow-up"
                value={npForm.secondary_status}
                onChange={(e) => setNpForm((s) => ({ ...s, secondary_status: e.target.value }))}
              />
              ...
              <textarea
                ...
                placeholder="Notes"
                value={npForm.notes}
                onChange={(e) => setNpForm((s) => ({ ...s, notes: e.target.value }))}
              />

              <div className="md:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/40 p-3">
                <div className="text-xs text-slate-400">Attachments (MVP UI)</div>
                <input
                  type="file"
                  multiple
                  onChange={(e) => setNpFiles(Array.from(e.target.files || []))}
                  className="mt-2 block text-sm text-slate-200"
                />
                <div className="mt-2 text-[11px] text-slate-500">
                  Attachment upload will wire in once the backend prospect-attachments endpoint is connected.
                </div>
                {npFiles.length ? (
                  <div className="mt-2 text-xs text-slate-300">
                    {npFiles.slice(0, 4).map((f) => (
                      <div key={f.name}>• {f.name}</div>
                    ))}
                    {npFiles.length > 4 ? <div>…and {npFiles.length - 4} more</div> : null}
                  </div>
                ) : null}
              </div>
            </div>
            ...
            <div className="mt-4 flex gap-2 flex-wrap">
              <button
                type="button"
                onClick={createProspectFromModal}
                disabled={npBusy}
                className={cx(
              ...
              >
                {npBusy ? "Creating..." : "Create Prospect"}
              </button>

              <button
                type="button"
                onClick={() => setNpModal({ open: false })}
            ...
            <div className="mt-4 grid lg:grid-cols-3 gap-4">
              <div className="lg:col-span-1 space-y-3">
                ...
                <div className="rounded-3xl border border-slate-800 bg-slate-950/40 p-4">
                  ...
                  <div className="text-xs text-slate-400 mt-1">If Status filter is set, it becomes default stage for blank stage_id rows.</div>
                  <button
                    type="button"
                    onClick={importCsv}
                ...
                <div className="rounded-3xl border border-slate-800 bg-slate-950/40 p-4">
                  ...
                  {!csvPreview.rows.length ? (
                    ...
                    <div className="mt-4 overflow-x-auto">
                      ...
                      <div className="text-[11px] text-slate-500 mt-2">Tip: Wrap notes in quotes if they contain commas.</div>
                    </div>
                  )}
                </div>
              </div>
