...
const emptyWorkspace = {
  name: "", manager_name: "", phone: "", office_email: "", tenant_email: "", reply_to_email: "", sender_name: "", website: "", office_address: "", email_signature: "",
};
...
function userDefaults(user) {
  ...
  return {
    ...
    phone: valueFrom(user, ["phone", "phone_number", "mobile"]),
    office_address: valueFrom(user, ["address", "street_address", "mailing_address"]),
  };
}
...
export default function PmSettings() {
  ...
  useEffect(() => {
    if (showMessages) return undefined;
    let active = true;
    (async () => {
      setLoading(true);
      setError("");
      try {
        ...
        if (creatingNew) {
          if (active) { setWorkspace(mergedDefaults); setWorkspaceId(""); setEditing(true); }
          return;
        }
        ...
        if (!active) return;
        if (err?.response?.status === 404) { setWorkspace(defaults); setWorkspaceId(""); setEditing(true); }
        else setError(errorMessage(err, "Could not load Property Management settings."));
      } finally {
        if (active) setLoading(false);
      }
    })();
    return () => { active = false; };
  }, [creatingNew, defaults, showMessages]);
      ...
      <main className="relative z-10 mx-auto max-w-4xl space-y-4 px-4 pb-[calc(13rem+env(safe-area-inset-bottom))] pt-5">
        <section className="rounded-[28px] border border-cyan-400/20 bg-[#07111f]/95 p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex flex-wrap gap-2"><Button tone="slate" onClick={() => nav("/pm/settings?view=messages")}>Open PM Messages</Button>{locked ? <Button tone="cyan" onClick={() => setEditing(true)}>Edit</Button> : null}</div>
          </div>
        ...
        <section className="rounded-[28px] border border-blue-500/20 bg-[#07111f]/90 p-4 sm:p-5">
          {loading ? <div className="text-sm text-slate-400">Loading portfolio information...</div> : <div className="grid gap-4 sm:grid-cols-2">
