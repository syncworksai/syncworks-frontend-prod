...
export default function PMPayments() {
  ...
  async function load() {
    try {
      ...
      setTenants(tenantRows); setEntries(list(e.data)); setSummary(s.data || { total_due:"0.00", tenants:[] }); setBillingSummary(b.data || { total_due:"0.00", total_past_due:"0.00", past_due_accounts:0, tenants:[] });
  ...
  async function saveEntry() {
    ...
    try {
      ...
      setEntry({ ...blank, tenant: entry.tenant }); setMessage("Ledger entry saved."); await load(); await loadBillingProfile(entry.tenant);
  ...
  async function saveBilling() {
    ...
    try {
      ...
      setBilling({ ...defaultBilling, ...(response.data?.profile || {}) }); setBillingAccount(response.data?.account || null); setMessage("Tenant billing rules saved."); await load();
  ...
  return <div className="min-h-screen bg-transparent text-slate-100"><main className="space-y-5 px-4 py-6 sm:px-6">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      <div className="rounded-3xl border border-rose-500/20 bg-[#07111f]/95 p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">Total Due</div><div className="mt-3 text-3xl font-black text-white">{money(billingSummary.total_due)}</div></div>
      <div className="rounded-3xl border border-rose-500/30 bg-[#07111f]/95 p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">Past Due</div><div className="mt-3 text-3xl font-black text-rose-200">{money(billingSummary.total_past_due)}</div></div>
      <div className="rounded-3xl border border-amber-500/25 bg-[#07111f]/95 p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">Past Due Accounts</div><div className="mt-3 text-3xl font-black text-amber-200">{billingSummary.past_due_accounts || 0}</div></div>
      <div className="rounded-3xl border border-cyan-500/15 bg-[#07111f]/95 p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">Tenant Accounts</div><div className="mt-3 text-3xl font-black text-white">{tenants.length}</div></div>
      <div className="rounded-3xl border border-fuchsia-500/20 bg-[#07111f]/95 p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">Selected Balance</div><div className="mt-3 text-3xl font-black text-white">{money(selectedBalance)}</div></div>
    </div>
    ...
    <section className="rounded-[28px] border border-cyan-500/20 bg-[#07111f]/95 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-[10px] font-black uppercase tracking-[.18em] text-cyan-300">Automatic billing</div><h2 className="mt-2 text-xl font-black">Tenant rent and fee schedule</h2><p className="mt-1 text-sm text-slate-400">Create rent, deposit, and late-fee charges so the balance reflects what is owed—not only what was paid.</p></div><div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-2xl border border-slate-800 bg-black/25 p-3"><div className="text-slate-500">Due</div><div className="mt-1 font-black text-white">{money(selectedBillingSummary?.amount_due)}</div></div><div className="rounded-2xl border border-rose-500/20 bg-black/25 p-3"><div className="text-slate-500">Past due</div><div className="mt-1 font-black text-rose-200">{money(selectedBillingSummary?.past_due)}</div></div><div className="rounded-2xl border border-amber-500/20 bg-black/25 p-3"><div className="text-slate-500">Late fees</div><div className="mt-1 font-black text-amber-200">{money(selectedBillingSummary?.late_fees_charged)}</div></div></div></div>
      <div className="mt-5 grid gap-4 lg:grid-cols-4"><Field label="Tenant account"><select className={inputClass} value={billingTenant} onChange={(e)=>setBillingTenant(e.target.value)}><option value="">Choose tenant</option>{tenants.map((t)=><option key={t.id} value={t.id}>{t.full_name}</option>)}</select></Field><Field label="Billing start date"><input type="date" className={inputClass} value={billing.billing_start_date || billing.lease_start || ""} onChange={(e)=>setBilling({...billing,billing_start_date:e.target.value})}/></Field><Field label="Rent due day"><input type="number" min="1" max="28" className={inputClass} value={billing.rent_due_day} onChange={(e)=>setBilling({...billing,rent_due_day:e.target.value})}/></Field><Field label="Grace period (days)"><input type="number" min="0" max="30" className={inputClass} value={billing.grace_days} onChange={(e)=>setBilling({...billing,grace_days:e.target.value})}/></Field><Field label="Late fee type"><select className={inputClass} value={billing.late_fee_type} onChange={(e)=>setBilling({...billing,late_fee_type:e.target.value})}><option value="FLAT">Flat dollar amount</option><option value="PERCENT">Percent of monthly rent</option></select></Field><Field label={billing.late_fee_type === "PERCENT" ? "Late fee percent" : "Late fee amount"}><input inputMode="decimal" className={inputClass} value={billing.late_fee_amount} onChange={(e)=>setBilling({...billing,late_fee_amount:e.target.value})}/></Field><Field label="Generate through"><input type="date" className={inputClass} value={throughDate} onChange={(e)=>setThroughDate(e.target.value)}/></Field><div className="grid grid-cols-2 gap-2"><Button tone="slate" onClick={saveBilling} disabled={saving}>{saving?"Saving...":"Save Rules"}</Button><Button tone="cyan" onClick={generateCharges} disabled={generating}>{generating?"Generating...":"Generate Charges"}</Button></div></div>
      ...
      <div className="mt-4 rounded-2xl border border-slate-800 bg-black/20 px-4 py-3 text-xs text-slate-400">Lease rent: <strong className="text-white">{money(billing.monthly_rent)}</strong> · Deposit: <strong className="text-white">{money(billing.security_deposit)}</strong> · Next due date: <strong className="text-white">{selectedBillingSummary?.next_due_date || "Not calculated"}</strong>{billing.section8 ? <> · Tenant portion: <strong className="text-white">{money(billing.tenant_portion)}</strong> · Housing assistance: <strong className="text-white">{money(billing.assistance_portion)}</strong></> : null}</div>
    </section>
