...
export default function PMPayments() {
  ...
  async function saveEdit() {
    ...
    try {
      const headers = { "X-PM-Workspace-ID": String(workspace.id) };
      await api.patch(`/pm-hub/ledger/${editing.id}/`, {
        entry_date: editing.entry_date,
        entry_type: editing.entry_type,
        amount: editing.amount,
        category: editing.category,
        payment_method: editing.entry_type === "PAYMENT" ? editing.payment_method : "",
      ...
      await load();
      await loadBillingProfile(String(tenantId));
    } catch (err) {
  ...
  async function deleteEntry(row) {
    ...
    try {
      ...
      await load();
      await loadBillingProfile(String(row.tenant));
    } catch (err) {
  ...
  function exportCsv() {
    ...
    link.href = url;
    link.download = "syncworks-tenant-ledger.csv";
    link.click();
  ...
  return <div className="min-h-screen bg-transparent text-slate-100"><main className="space-y-5 px-4 py-6 sm:px-6">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {[
        ["Total Due", billingSummary.total_due, true],
        ["Past Due", billingSummary.total_past_due, true],
        ["Past Due Accounts", billingSummary.past_due_accounts, false],
        ["Tenant Accounts", tenants.length, false],
        ["Selected Balance", selectedBalance, true],
      ].map(([label, value, isMoney]) => <div key={label} className="rounded-3xl border border-cyan-500/15 bg-[#07111f]/95 p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">{label}</div><div className="mt-3 text-3xl font-black text-white">{isMoney ? money(value) : value || 0}</div></div>)}
    </div>
    ...
    <section className="rounded-[28px] border border-cyan-500/20 bg-[#07111f]/95 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><div className="text-[10px] font-black uppercase tracking-[.18em] text-cyan-300">Automatic billing</div><h2 className="mt-2 text-xl font-black">Tenant rent and fee schedule</h2><p className="mt-1 text-sm text-slate-400">Set the date your company began managing the account, then generate only the charges you need.</p></div>
        <div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-2xl border border-slate-800 bg-black/25 p-3"><div className="text-slate-500">Due</div><div className="mt-1 font-black text-white">{money(selectedBillingSummary?.amount_due)}</div></div><div className="rounded-2xl border border-rose-500/20 bg-black/25 p-3"><div className="text-slate-500">Past due</div><div className="mt-1 font-black text-rose-200">{money(selectedBillingSummary?.past_due)}</div></div><div className="rounded-2xl border border-amber-500/20 bg-black/25 p-3"><div className="text-slate-500">Late fees</div><div className="mt-1 font-black text-amber-200">{money(selectedBillingSummary?.late_fees_charged)}</div></div></div>
      </div>
      <div className="mt-5 grid gap-4 lg:grid-cols-4">
        <Field label="Tenant account"><select className={inputClass} value={billingTenant} onChange={(event) => setBillingTenant(event.target.value)}><option value="">Choose tenant</option>{tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.full_name}</option>)}</select></Field>
        <Field label="PM management start date"><input type="date" className={inputClass} value={billing.billing_start_date || ""} onChange={(event) => setBilling({ ...billing, billing_start_date: event.target.value })} /></Field>
        <Field label="Rent due day"><input type="number" min="1" max="28" className={inputClass} value={billing.rent_due_day} onChange={(event) => setBilling({ ...billing, rent_due_day: event.target.value })} /></Field>
        <Field label="Grace period (days)"><input type="number" min="0" max="30" className={inputClass} value={billing.grace_days} onChange={(event) => setBilling({ ...billing, grace_days: event.target.value })} /></Field>
        <Field label="Late fee type"><select className={inputClass} value={billing.late_fee_type} onChange={(event) => setBilling({ ...billing, late_fee_type: event.target.value })}><option value="FLAT">Flat dollar amount</option><option value="PERCENT">Percent of monthly rent</option></select></Field>
        <Field label={billing.late_fee_type === "PERCENT" ? "Late fee percent" : "Late fee amount"}><input inputMode="decimal" className={inputClass} value={billing.late_fee_amount} onChange={(event) => setBilling({ ...billing, late_fee_amount: event.target.value })} /></Field>
        <Field label="Generate through"><input type="date" className={inputClass} value={throughDate} onChange={(event) => setThroughDate(event.target.value)} /></Field>
      ...
      <div className="mt-4 rounded-2xl border border-slate-800 bg-black/20 px-4 py-3 text-xs text-slate-400">Wrong charge? Use <strong className="text-white">Edit</strong> or <strong className="text-white">Delete</strong> directly on that ledger row. Deleting removes the line from the balance instead of adding a credit.</div>
    </section>

    <div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
      <section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/90 p-5">
        <h2 className="text-lg font-black">Add ledger entry</h2><p className="mt-1 text-xs text-slate-500">Back-date charges and payments to reconstruct an existing ledger.</p>
        <div className="mt-5 grid gap-4">
          <Field label="Tenant"><select className={inputClass} value={entry.tenant} onChange={(event) => setEntry({ ...entry, tenant: event.target.value })}><option value="">Choose tenant</option>{tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.full_name} · balance {money(tenant.balance)}</option>)}</select></Field>
          <div className="grid grid-cols-2 gap-3"><Field label="Date"><input type="date" className={inputClass} value={entry.entry_date} onChange={(event) => setEntry({ ...entry, entry_date: event.target.value })} /></Field><Field label="Type"><select className={inputClass} value={entry.entry_type} onChange={(event) => setEntry({ ...entry, entry_type: event.target.value })}>{entryTypes.map((value) => <option key={value}>{value}</option>)}</select></Field></div>
          <div className="grid grid-cols-2 gap-3"><Field label="Amount"><input inputMode="decimal" className={inputClass} value={entry.amount} onChange={(event) => setEntry({ ...entry, amount: event.target.value })} /></Field><Field label="Category"><select className={inputClass} value={entry.category} onChange={(event) => setEntry({ ...entry, category: event.target.value })}>{categories.map((value) => <option key={value}>{value.replaceAll("_", " ")}</option>)}</select></Field></div>
          {entry.entry_type === "PAYMENT" ? <Field label="Payment method"><select className={inputClass} value={entry.payment_method} onChange={(event) => setEntry({ ...entry, payment_method: event.target.value })}>{paymentMethods.map((value) => <option key={value}>{value.replaceAll("_", " ")}</option>)}</select></Field> : null}
          <Field label="Reference / check number"><input className={inputClass} value={entry.reference} onChange={(event) => setEntry({ ...entry, reference: event.target.value })} /></Field>
          <Field label="Memo"><textarea rows="3" className={inputClass} value={entry.memo} onChange={(event) => setEntry({ ...entry, memo: event.target.value })} /></Field>
          <Button tone="cyan" onClick={saveEntry} disabled={saving}>{saving ? "Saving..." : "Save Entry"}</Button>
        </div>
      </section>

      <section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/90 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-black">Tenant ledger</h2><p className="mt-1 text-xs text-slate-500">Edit or delete each line while setting up the correct tenant balance.</p></div><div className="flex gap-2"><select className={inputClass} style={{ width: 240 }} value={filterTenant} onChange={(event) => setFilterTenant(event.target.value)}><option value="">All tenants</option>{tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.full_name}</option>)}</select><Button tone="slate" onClick={exportCsv}>Export CSV</Button></div></div>
        <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead><tr className="border-b border-slate-700 text-xs uppercase tracking-wider text-slate-500">{["Date", "Tenant", "Type", "Category", "Reference", "Amount", "Actions"].map((heading) => <th key={heading} className="px-3 py-3">{heading}</th>)}</tr></thead><tbody>{entries.map((row) => <tr key={row.id} className="border-b border-slate-800"><td className="px-3 py-3">{row.entry_date}</td><td className="px-3 py-3 font-bold text-white">{row.tenant_name}</td><td className="px-3 py-3">{row.entry_type}</td><td className="px-3 py-3">{row.category}</td><td className="max-w-[220px] truncate px-3 py-3 text-xs text-slate-400">{row.reference || "—"}</td><td className={`px-3 py-3 font-black ${["PAYMENT", "CREDIT"].includes(row.entry_type) ? "text-emerald-300" : "text-rose-200"}`}>{["PAYMENT", "CREDIT"].includes(row.entry_type) ? "−" : "+"}{money(row.amount)}</td><td className="px-3 py-3"><div className="flex gap-2"><button className="rounded-xl border border-cyan-400/30 px-3 py-2 text-xs font-bold text-cyan-100" onClick={() => beginEdit(row)}>Edit</button><button className="rounded-xl border border-rose-400/30 px-3 py-2 text-xs font-bold text-rose-100" onClick={() => deleteEntry(row)}>Delete</button></div></td></tr>)}</tbody></table>{!entries.length ? <div className="py-12 text-center text-sm text-slate-500">No ledger entries yet.</div> : null}</div>
      </section>
    </div>
