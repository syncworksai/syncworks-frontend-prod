    ...
    <div className="min-h-screen bg-[#02060c] pb-28 text-slate-100">
      ...
      <main className="mx-auto max-w-6xl space-y-4 px-3 py-4 sm:px-5">
        ...
        <section className="overflow-hidden rounded-[2rem] border border-cyan-400/20 bg-[radial-gradient(circle_at_85%_5%,rgba(34,211,238,.18),transparent_34%),radial-gradient(circle_at_10%_105%,rgba(139,92,246,.16),transparent_35%),#07111f] p-5 sm:p-7">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[.15em] text-cyan-100"><Trophy className="h-4 w-4 text-amber-300" />Sports inside Social</div>
          <h1 className="mt-4 max-w-3xl text-3xl font-black sm:text-5xl">Teams, leagues, game day and stats.</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Social owns people and groups. Sports adds leagues, rosters, schedules, live scoring and analytics without duplicating identity or calendar.</p>
        </section>
          ...
          <>
            ...
            <section className="rounded-[1.65rem] border border-white/10 bg-[#07111f] p-4 sm:p-5">
              <div className="flex items-start justify-between gap-3">
                <div><h2 className="font-black text-white">Social team groups</h2><p className="mt-1 text-sm text-slate-400">Choose a Team group to open or set up its sports dashboard.</p></div>
                ...
                {teamGroups.map((group) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-5 flex items-center justify-between text-xs font-black text-cyan-100"><span>{sports ? "Open sports dashboard" : "Set up softball"}</span><ChevronRight className="h-4 w-4" /></div>
                    </button>
                  );
                })}
              </div>
              {!teamGroups.length ? (
                <div className="mt-4 rounded-2xl border border-dashed border-white/10 p-6 text-center">
                  ...
                  <p className="mt-1 text-sm text-slate-500">Go back to Social → Groups → Create and choose Team. Then it will appear here.</p>
