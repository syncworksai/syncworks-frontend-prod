...
const CALENDAR_CACHE_KEY = "syncworks_calendar_last_good_v2";
...
function eventDriveUrl(event) {
  ...
  const apple = typeof navigator !== "undefined" && /iPhone|iPad|iPod|Macintosh/i.test(navigator.userAgent || "");
  if (apple) {
...
function parseSmartCapture(text) {
  ...
  return {
    ...
    date: ymd(date),
    end_date: ymd(endMinutes >= 1440 ? addDays(date, 1) : date),
...
function ActionButton({ children, onClick, tone = "slate", disabled = false }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex min-h-9 items-center justify-center gap-1.5 rounded-xl border px-3 text-[10px] font-black transition disabled:opacity-40 ${tones[tone] || tones.slate}`}
    >
      {children}
    </button>
  );
...
function EventBlock({ event, conflict, onEdit, onServiceResponse, onMoveFlexible, onToggleTask }) {
  ...
  return (
    <div className={`rounded-xl border p-2.5 ${sourceTone(event)} ${done ? "opacity-55" : ""}`}>
      <div className="flex items-start gap-2">
        ...
        {editableEvent(event) ? (
          <button type="button" onClick={() => onEdit(event)} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-current/20" aria-label={`Edit ${event.title}`}>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        ...
        {pending ? (
          <>
            <ActionButton onClick={() => onServiceResponse(event, "ACCEPT")} tone="emerald"><Check className="h-3 w-3" />Accept</ActionButton>
            <ActionButton onClick={() => onServiceResponse(event, "REQUEST_CHANGE")} tone="amber"><Clock className="h-3 w-3" />Different time</ActionButton>
          </>
        ) : null}
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-white/10">
      <div className="grid grid-cols-7 border-b border-white/10 bg-black/20">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <div key={day} className="p-2 text-center text-[9px] font-black uppercase text-slate-500">{day}</div>)}
      </div>
      <div className="grid grid-cols-7">
        {days.map((day) => {
          ...
          const healthCount = rows.filter((row) => sourceKey(row) === "HEALTH").length;
          return (
            <button type="button" key={ymd(day)} onClick={() => onSelectDay(day)} className={`min-h-[82px] border-b border-r border-white/[.07] p-1.5 text-left sm:min-h-28 sm:p-2 ${inMonth ? "bg-white/[.015]" : "bg-black/20 opacity-40"}`}>
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {rows.slice(0, 4).map((event) => <span key={event.id} className={`h-1.5 w-1.5 rounded-full ${sourceKey(event) === "HEALTH" ? "bg-emerald-400" : isTask(event) ? "bg-violet-400" : "bg-cyan-400"}`} />)}
              </div>
              <span className="mt-1 block text-[8px] font-bold text-slate-500">{rows.length ? `${rows.length} scheduled${healthCount ? ` · ${healthCount} workout` : ""}` : "Open"}</span>
            </button>
          );
        })}
      </div>
...
function DayView({ day, events, conflictIds, onCreate, onEdit, onServiceResponse, onMoveFlexible, onToggleTask }) {
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-black/15">
      {allDay.length ? <div className="grid grid-cols-[58px_minmax(0,1fr)] border-b border-white/10"><div className="p-2 text-right text-[9px] font-black uppercase text-slate-500">All day</div><div className="space-y-1.5 border-l border-white/10 p-2">{allDay.map((event) => <EventBlock key={event.id} event={event} conflict={conflictIds.has(event.id)} onEdit={onEdit} onServiceResponse={onServiceResponse} onMoveFlexible={onMoveFlexible} onToggleTask={onToggleTask} />)}</div></div> : null}
      {hours.map((hour) => {
        const rows = timed.filter((event) => new Date(event.start_at).getHours() === hour);
        const isNow = ymd(day) === ymd() && new Date().getHours() === hour;
        return (
          <div key={hour} className={`grid min-h-[58px] grid-cols-[58px_minmax(0,1fr)] border-b border-white/[.07] ${isNow ? "bg-cyan-500/[.04]" : ""}`}>
            <button type="button" onClick={() => onCreate(day, hour)} className={`p-2 text-right text-[10px] font-bold ${isNow ? "text-cyan-300" : "text-slate-500"}`}>{new Date(2000, 0, 1, hour).toLocaleTimeString("en-US", { hour: "numeric" })}</button>
            <div className="space-y-1.5 border-l border-white/10 p-1.5">{rows.map((event) => <EventBlock key={event.id} event={event} conflict={conflictIds.has(event.id)} onEdit={onEdit} onServiceResponse={onServiceResponse} onMoveFlexible={onMoveFlexible} onToggleTask={onToggleTask} />)}</div>
          </div>
        );
      })}
    </div>
...
export default function CalendarPage() {
  const [events, setEvents] = useState(() => {
    try {
      return safeList(JSON.parse(window.localStorage.getItem(CALENDAR_CACHE_KEY) || "[]"));
    } catch {
      return [];
    }
  });
  ...
  const loadEvents = useCallback(async ({ silent = false } = {}) => {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        setEvents(rows);
        try { window.localStorage.setItem(CALENDAR_CACHE_KEY, JSON.stringify(rows)); } catch { /* optional cache */ }
        if (!silent) setNotice("");
        setLoading(false);
        return rows;
      } catch (loadError) {
        ...
        if (attempt < 2) await sleep(attempt === 0 ? 450 : 1200);
      }
    }
    setLoading(false);
    if (events.length) {
    ...
    setError(lastError?.response?.data?.detail || "Calendar could not reconnect yet. Tap refresh to retry.");
    return [];
  }, [events]);
  ...
  useEffect(() => {
    ...
    if (oauth === "connected") {
      setNotice(`${provider === "microsoft" ? "Outlook" : "Google"} connected. Calendar is refreshing.`);
      loadConnections();
      loadEvents({ silent: true });
    } else if (oauth === "error") {
      setError(`Could not finish ${provider || "calendar"} connection. Open Connect calendars to try again.`);
    }
  ...
  const lastMeasurement = useMemo(() => {
    const logs = Array.isArray(healthProfile?.progress_json) ? healthProfile.progress_json : [];
    return logs.find((item) => item?.measurements && Object.values(item.measurements || {}).some(Boolean)) || null;
  }, [healthProfile]);
  ...
  function openHealth(target) {
    try { window.localStorage.setItem(HEALTH_OPEN_ACTION_KEY, target); } catch { /* optional handoff */ }
  ...
  async function useNextWorkoutToday() {
    ...
    try {
      ...
      scheduled.setMinutes(minutes, 0, 0);
      if (minutes === 60) scheduled.setHours(now.getHours() + 1, 0, 0, 0);
      const moved = {
        ...nextHealthPlan,
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100">
      ...
      <main className="mx-auto w-full max-w-[1680px] px-3 py-3 pb-28 sm:px-4 lg:px-6 lg:pb-8">
        ...
        <section className="rounded-[1.7rem] border border-cyan-400/20 bg-[radial-gradient(circle_at_top_right,rgba(139,92,246,.12),transparent_34%),linear-gradient(145deg,rgba(8,47,73,.28),rgba(2,6,23,.96))] p-3 sm:p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h1 className="mt-1 text-lg font-black text-white sm:text-xl">Your schedule is the first thing you see</h1>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <button type="button" onClick={() => { setWeekStart(startOfWeek(new Date())); setMonthStart(new Date(new Date().getFullYear(), new Date().getMonth(), 1)); setSelectedDay(new Date()); }} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-3 text-[11px] font-black text-slate-200"><CalendarDays className="h-4 w-4" />Today</button>
              <button type="button" onClick={() => { loadEvents(); loadConnections(); loadHealth(); }} className="grid h-10 w-10 place-items-center rounded-xl border border-white/10 text-slate-400"><RefreshCw className={`h-4 w-4 ${loading || connectionLoading || healthLoading ? "animate-spin" : ""}`} /></button>
            </div>
          </div>
            ...
            {upcoming.length ? upcoming.map((event) => {
              ...
              return (
                <div key={event.id} className={`rounded-xl border p-3 ${sourceTone(event)}`}>
                  ...
                  {location ? <div className="mt-1 truncate text-[9px] opacity-70">{location}</div> : <div className="mt-1 text-[9px] font-bold text-amber-200">No address yet</div>}
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {editableEvent(event) ? <ActionButton onClick={() => editEvent(event)} tone={location ? "slate" : "amber"}><Pencil className="h-3 w-3" />{location ? "Edit" : "Add address"}</ActionButton> : null}
        ...
        {pendingServices.length ? <section className="mt-4 rounded-[1.5rem] border border-amber-400/25 bg-amber-500/[.06] p-4"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] text-amber-200"><AlertTriangle className="h-4 w-4" />Service schedule changes</div><div className="mt-3 grid gap-2 md:grid-cols-2">{pendingServices.map((event) => <EventBlock key={event.id} event={event} conflict={false} onEdit={editEvent} onServiceResponse={respondToService} onMoveFlexible={moveFlexible} onToggleTask={toggleTask} />)}</div></section> : null}

        <div className="mt-4 grid gap-4 xl:grid-cols-3">
          <section className="rounded-[1.5rem] border border-emerald-400/20 bg-emerald-500/[.05] p-4">
            <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] text-emerald-200"><HeartPulse className="h-4 w-4" />Health today</div><h2 className="mt-2 text-lg font-black text-white">{todayWorkoutEvent || todayHealthPlan ? (todayWorkoutEvent?.title || todayHealthPlan?.workout_name || todayHealthPlan?.name || "Workout planned") : "No workout planned today"}</h2><p className="mt-1 text-xs leading-5 text-slate-400">{todayWorkoutEvent || todayHealthPlan ? "Your workout is on the calendar. You can start it or adjust the plan from here." : "An open Health day should be actionable. Build a workout, continue the next session, or ask the coach to adjust today."}</p></div><Dumbbell className="h-6 w-6 text-emerald-300" /></div>
            <div className="mt-4 flex flex-wrap gap-2">{todayWorkoutEvent ? <a href="/customer/health" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-emerald-300/30 bg-emerald-500/15 px-3 text-[10px] font-black text-emerald-100"><Dumbbell className="h-3.5 w-3.5" />Start workout</a> : <ActionButton onClick={() => openHealth("plan-today")} tone="emerald"><Dumbbell className="h-3.5 w-3.5" />Need workout</ActionButton>}{!todayWorkoutEvent && !todayHealthPlan && nextHealthPlan ? <ActionButton onClick={useNextWorkoutToday} tone="cyan" disabled={healthLoading}><Zap className="h-3.5 w-3.5" />Use next workout today</ActionButton> : null}<ActionButton onClick={() => openHealth("coach-chat")} tone="violet"><Sparkles className="h-3.5 w-3.5" />AI adjust</ActionButton><ActionButton onClick={() => openHealth("quick-log")}><Scale className="h-3.5 w-3.5" />Weigh in</ActionButton><ActionButton onClick={() => openHealth("progress")}><Ruler className="h-3.5 w-3.5" />Measurements</ActionButton><ActionButton onClick={() => openHealth("planner")}><CalendarDays className="h-3.5 w-3.5" />Workout plan</ActionButton></div>
            <div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-[9px] font-black uppercase text-slate-500">Weight</div><div className="mt-1 text-sm font-black text-white">{healthSnapshot?.weight || healthProfile?.profile_json?.weight || "Log today"}</div></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-[9px] font-black uppercase text-slate-500">Measurements</div><div className="mt-1 text-sm font-black text-white">{lastMeasurement?.date || lastMeasurement?.ymd || "Add baseline"}</div></div></div>
          </section>
          ...
          <section className="rounded-[1.5rem] border border-cyan-400/20 bg-cyan-500/[.04] p-4">
            ...
            {nextEvent ? <><h2 className="mt-2 text-lg font-black text-white">{nextEvent.title}</h2><div className="mt-1 text-xs text-slate-400">{new Date(nextEvent.start_at).toLocaleString("en-US", { weekday: "long", hour: "numeric", minute: "2-digit" })}</div>{eventLocation(nextEvent) ? <div className="mt-2 text-xs leading-5 text-slate-300">{eventLocation(nextEvent)}</div> : <div className="mt-2 rounded-xl border border-amber-400/20 bg-amber-500/10 p-2.5 text-xs font-bold text-amber-100">No address saved. Add the exact address now so routing is ready when you leave.</div>}<div className="mt-3 flex flex-wrap gap-2">{editableEvent(nextEvent) ? <ActionButton onClick={() => editEvent(nextEvent)} tone={eventLocation(nextEvent) ? "slate" : "amber"}><Pencil className="h-3.5 w-3.5" />{eventLocation(nextEvent) ? "Edit details" : "Add address"}</ActionButton> : null}{nextDriveUrl ? <a href={nextDriveUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center gap-2 rounded-xl border border-cyan-400/25 bg-cyan-500/10 px-3 text-[10px] font-black text-cyan-100"><Navigation className="h-3.5 w-3.5" />Drive / CarPlay</a> : null}{eventMapsUrl(nextEvent) ? <a href={eventMapsUrl(nextEvent)} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center gap-2 rounded-xl border border-white/10 px-3 text-[10px] font-black text-slate-200"><MapPin className="h-3.5 w-3.5" />Map</a> : null}</div><p className="mt-3 text-[10px] leading-4 text-slate-500">On iPhone, Drive hands the destination to Apple Maps. When the phone is connected to CarPlay, supported navigation displays in the vehicle.</p></> : <div className="mt-3 text-xs text-slate-500">Nothing upcoming yet.</div>}
          </section>
          ...
          <section className="rounded-[1.5rem] border border-violet-400/20 bg-violet-500/[.04] p-4">
            ...
            <div className="mt-3 space-y-2">{tasks.length ? tasks.slice(0, 4).map((task) => <EventBlock key={task.id} event={task} conflict={todayConflictIds.has(task.id)} onEdit={editEvent} onServiceResponse={respondToService} onMoveFlexible={moveFlexible} onToggleTask={toggleTask} />) : <button type="button" onClick={() => openNew("TASK")} className="w-full rounded-xl border border-dashed border-violet-400/20 p-4 text-xs font-bold text-violet-200">+ Add your first task</button>}</div>
          </section>
        </div>
