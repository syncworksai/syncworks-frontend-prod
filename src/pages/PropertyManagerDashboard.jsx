...
function Metric({ label, value, hint, tone = "cyan" }) {
  ...
  return (
    <div className="rounded-[28px] border border-blue-500/20 bg-[#07111f]/95 p-5">
      <div className="flex items-center justify-between gap-3"><div className="text-xs uppercase tracking-[0.18em] text-slate-500">{label}</div><span className={`rounded-full border bg-black/20 px-2.5 py-1 text-[10px] font-bold ${tones[tone]}`}>LIVE</span></div>
...
function Panel({ title, subtitle, children, action }) {
  return (
    <section className="rounded-[28px] border border-blue-500/20 bg-[#07111f]/90">
      <div className="flex items-start justify-between gap-3 border-b border-slate-800/70 px-5 py-4"><div><h2 className="text-sm font-semibold text-white">{title}</h2>{subtitle ? <p className="mt-1 text-xs text-slate-500">{subtitle}</p> : null}</div>{action}</div>
...
export default function PropertyManagerDashboard() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      const [propertiesResult, workOrdersResult] = await Promise.allSettled([
              ...
              <div className="grid gap-3">
                {properties.map((property) => (
                  <button key={property.id} type="button" onClick={() => nav(`/pm/properties/${property.id}`)} className="w-full rounded-3xl border border-blue-500/15 bg-black/25 p-4 text-left transition hover:border-cyan-400/40">
                    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-sm font-semibold text-white">{property.name || "Unnamed Property"}</div><div className="mt-1 truncate text-xs text-slate-500">{[property.address, property.city, property.state].filter(Boolean).join(", ") || "Address not entered"}</div></div><span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-[10px] text-emerald-200">{String(property.status || "HEALTHY").replaceAll("_", " ")}</span></div>
                  </button>
                ))}
              </div>
            ) : (
              <div className="rounded-3xl border border-dashed border-slate-800 bg-black/20 p-8 text-center"><div className="text-sm text-slate-400">No properties have been added.</div><div className="mt-4"><Button tone="cyan" onClick={() => nav(workspace?.id ? "/pm/properties/new" : "/pm/settings")}>{workspace?.id ? "Create First Property" : "Set Up Portfolio"}</Button></div></div>
            )}
          </Panel>

          <div className="space-y-6">
            <Panel title="Operations" subtitle="Direct access to daily Property Management tools."><div className="grid gap-3"><Button tone="cyan" onClick={() => nav("/pm/properties/new")}>Add Property</Button><Button tone="slate" onClick={() => nav("/pm/settings")}>Portfolio Settings</Button><Button tone="slate" onClick={() => nav("/pm/employees")}>Team</Button><Button tone="slate" onClick={() => nav("/pm/calendar")}>Schedule</Button></div></Panel>
