...
export default function PMMessages() {
  ...
  const [view, setView] = useState(() => new URLSearchParams(location.search).get("tab") === "occupancy" ? "records" : "messages");
  ...
  async function load() {
    ...
    try {
      ...
      const [messageRes, tenantRes, propertyRes, occupancyRes, caseRes] = await Promise.all([
        api.get("/pm-hub/messages/", { headers, params }), api.get("/pm-hub/tenants/", { headers }), api.get("/pm-hub/properties/", { headers }), api.get("/pm-hub/occupancies/", { headers }), api.get("/pm-hub/tenant-cases/", { headers }),
      ]);
      const rows = list(messageRes.data);
      setThreads(rows); setTenants(list(tenantRes.data)); setProperties(list(propertyRes.data)); setOccupancies(list(occupancyRes.data)); setCases(list(caseRes.data));
      setSelected((current) => rows.find((item) => item.id === current?.id) || rows[0] || null);
    } catch (error) { setNotice(error?.response?.data?.detail || "Could not load PM records."); }
    finally { setLoading(false); }
  }

  useEffect(() => { load(); }, [category]);
  useEffect(() => {
    if (query.get("tab") === "occupancy") setView("records");
    if (query.get("compose") === "1") setCompose(true);
  }, [query]);

  const counts = useMemo(() => categories.reduce((acc, item) => { acc[item] = item === "ALL" ? threads.length : threads.filter((thread) => thread.category === item).length; return acc; }, {}), [threads]);
  const activeOccupancies = occupancies.filter((item) => ["ACTIVE", "NOTICE_GIVEN"].includes(item.status));
  const requestedCase = String(query.get("case") || "").toUpperCase();
  const visibleCases = requestedCase === "EVICTIONS" ? cases.filter((item) => item.case_type === "EVICTION") : requestedCase === "COLLECTIONS" ? cases.filter((item) => item.case_type === "COLLECTIONS") : cases;
  ...
  async function sendReply() {
    ...
    setReply(""); setNotice("Internal app reply saved."); await load();
  }

  function openEmail() {
    if (!selected || selected.internal_only) return;
    setEmailForm({ to: selected.requester_email || "", subject: selected.subject || "", body: reply || "" });
    setEmailOpen(true);
  }
  ...
  async function sendEmail(event) {
    ...
    const headers = { "X-PM-Workspace-ID": String(workspace.id) };
    const response = await api.post(`/pm-hub/messages/${selected.id}/email/`, emailForm, { headers });
    setEmailOpen(false); setReply(""); setNotice(`${response.data?.detail || "Email sent."} Reference: ${response.data?.reference || "created"}`); await load();
  }

  async function createThread(event) {
    event.preventDefault(); const headers = { "X-PM-Workspace-ID": String(workspace.id) };
  ...
  async function closeOccupancy(item, evicted = false) {
    ...
    const headers = { "X-PM-Workspace-ID": String(workspace.id) };
    const response = await api.post(`/pm-hub/occupancies/${item.id}/move-out/`, { evicted, move_out_reason: reason }, { headers });
    setNotice(response.data?.detail || "Occupancy closed. Tenant history remains available."); await load();
  }
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6">
    ...
    <section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/92 p-4 sm:p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black text-white">PM Communications & Tenant Records</h2><p className="mt-1 text-xs text-slate-500">App messages, external email, occupancy history, move-outs, eviction, and collections without overwriting history.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setCompose(true)} className="min-h-11 rounded-2xl bg-cyan-400 px-5 text-sm font-black text-slate-950">+ New Message</button><button type="button" onClick={() => setOccupancyOpen(true)} className="min-h-11 rounded-2xl border border-fuchsia-400/30 bg-fuchsia-500/15 px-5 text-sm font-black text-fuchsia-100">Connect Occupancy</button><button type="button" onClick={() => setCaseOpen(true)} className="min-h-11 rounded-2xl border border-amber-400/30 bg-amber-500/10 px-5 text-sm font-black text-amber-100">Open Case</button></div></div><div className="mt-4 flex gap-2"><button type="button" onClick={() => setView("messages")} className={`rounded-xl px-4 py-2 text-xs font-black ${view === "messages" ? "bg-cyan-400 text-slate-950" : "border border-slate-700 text-slate-400"}`}>Messages</button><button type="button" onClick={() => setView("records")} className={`rounded-xl px-4 py-2 text-xs font-black ${view === "records" ? "bg-cyan-400 text-slate-950" : "border border-slate-700 text-slate-400"}`}>Occupancy & Cases</button></div></section>

    {view === "messages" ? <><section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/92 p-4"><div className="flex gap-2 overflow-x-auto pb-1">{categories.map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-xs font-black ${category === item ? "border-cyan-300 bg-cyan-400 text-slate-950" : "border-slate-700 bg-black/25 text-slate-400"}`}>{item.replace("_", " ")}{counts[item] ? ` ${counts[item]}` : ""}</button>)}</div><div className="mt-4 flex gap-2"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tenant, property, subject..." className={`${inputClass} flex-1`} /><button type="button" onClick={load} className="rounded-2xl border border-cyan-400/25 bg-cyan-500/10 px-4 text-sm font-black text-cyan-100">Search</button></div></section>
      ...
      <section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/92 p-5">{selected ? <><div className="border-b border-slate-800 pb-4"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-300">{selected.category}</div><h3 className="mt-2 text-2xl font-black text-white">{selected.subject}</h3><div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">{selected.requester_email ? <span>Email: {selected.requester_email}</span> : null}{selected.tenant_name ? <span>Tenant: {selected.tenant_name}</span> : null}{selected.property_name ? <span>Property: {selected.property_name}</span> : null}{selected.work_order ? <span>Work order #{selected.work_order}</span> : null}{selected.tenant_case ? <span>Case #{selected.tenant_case}</span> : null}</div></div><div className="my-5 space-y-3">{selected.messages?.map((message) => <div key={message.id} className={`rounded-2xl border p-4 ${["PM", "INTERNAL"].includes(message.sender_role) ? "ml-4 border-cyan-500/20 bg-cyan-500/10" : "mr-4 border-slate-700 bg-black/25"}`}><div className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">{message.sender_name} · {message.sender_role}</div><div className="mt-2 whitespace-pre-wrap text-sm text-slate-200">{message.body}</div></div>)}</div><textarea value={reply} onChange={(e) => setReply(e.target.value)} placeholder="Write a reply or email..." className="min-h-24 w-full rounded-2xl border border-slate-700 bg-black/30 p-4 text-sm text-white" /><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={sendReply} className="min-h-11 rounded-2xl bg-cyan-400 px-5 text-sm font-black text-slate-950">Save App Reply</button><button type="button" disabled={selected.internal_only} onClick={openEmail} className="min-h-11 rounded-2xl border border-fuchsia-400/30 bg-fuchsia-500/15 px-5 text-sm font-black text-fuchsia-100 disabled:opacity-40">Send Email</button><span className="self-center text-xs text-slate-500">Internal conversations cannot be emailed.</span></div></> : <div className="py-16 text-center text-sm text-slate-500">Select a conversation.</div>}</section></div></> :
      <div className="grid gap-5 xl:grid-cols-2"><section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/92 p-5"><div className="flex items-center justify-between"><div><h3 className="text-lg font-black text-white">Current Occupancies</h3><p className="mt-1 text-xs text-slate-500">Evicted automatically opens a case, collections thread, and make-ready work order.</p></div><span className="rounded-full border border-cyan-400/25 px-3 py-1 text-xs text-cyan-200">{activeOccupancies.length} active</span></div><div className="mt-4 space-y-3">{activeOccupancies.length ? activeOccupancies.map((item) => <div key={item.id} className="rounded-2xl border border-slate-800 bg-black/25 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-black text-white">{item.tenant_name}</div><div className="mt-1 text-xs text-slate-500">{item.property_name}{item.unit_label ? ` · ${item.unit_label}` : ""} · Since {item.move_in_date || "unknown"}</div></div><div className="flex gap-2"><button type="button" onClick={() => closeOccupancy(item, false)} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-black text-slate-200">Move Out</button><button type="button" onClick={() => closeOccupancy(item, true)} className="rounded-xl border border-rose-400/30 bg-rose-500/10 px-3 py-2 text-xs font-black text-rose-100">Evicted</button></div></div></div>) : <div className="rounded-2xl border border-dashed border-slate-700 p-7 text-center text-sm text-slate-500">No active occupancies.</div>}</div></section>
      <section className="rounded-[28px] border border-cyan-500/15 bg-[#07111f]/92 p-5"><div className="flex items-center justify-between"><div><h3 className="text-lg font-black text-white">{requestedCase === "EVICTIONS" ? "Eviction Cases" : requestedCase === "COLLECTIONS" ? "Collection Cases" : "Tenant Cases"}</h3><p className="mt-1 text-xs text-slate-500">Eviction, collections, legal, and payment-plan records remain attached to the former tenant.</p></div><span className="rounded-full border border-amber-400/25 px-3 py-1 text-xs text-amber-200">{visibleCases.filter((item) => item.status !== "CLOSED").length} open</span></div><div className="mt-4 space-y-3">{visibleCases.length ? visibleCases.map((item) => <div key={item.id} className="rounded-2xl border border-slate-800 bg-black/25 p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-black text-white">{item.tenant_name}</div><div className="mt-1 text-xs text-slate-500">{item.case_type.replaceAll("_", " ")} · {item.property_name || "Tenant record"}</div></div><span className="rounded-full border border-amber-400/25 px-2 py-1 text-[10px] font-black text-amber-200">{item.status.replaceAll("_", " ")}</span></div><div className="mt-3 grid grid-cols-2 gap-3 text-xs"><div><span className="text-slate-500">Current balance</span><div className="mt-1 font-black text-white">${Number(item.current_balance || 0).toFixed(2)}</div></div><div><span className="text-slate-500">Agency</span><div className="mt-1 font-bold text-white">{item.agency_name || "Not assigned"}</div></div></div></div>) : <div className="rounded-2xl border border-dashed border-slate-700 p-7 text-center text-sm text-slate-500">No matching tenant cases.</div>}</div></section></div>}
    ...
    {emailOpen ? <Modal title="Send External Email" onClose={() => setEmailOpen(false)}><form onSubmit={sendEmail}><div className="rounded-2xl border border-amber-400/20 bg-amber-500/10 p-3 text-xs leading-5 text-amber-100">This sends outside SyncWorks and saves a copy in this conversation. The subject receives an SW-PM reference so future replies can be matched. Internal notes are never included.</div><div className="mt-4 grid gap-3"><input required type="email" value={emailForm.to} onChange={(e) => setEmailForm({ ...emailForm, to: e.target.value })} placeholder="Recipient email" className={inputClass} /><input required value={emailForm.subject} onChange={(e) => setEmailForm({ ...emailForm, subject: e.target.value })} placeholder="Subject" className={inputClass} /><textarea required value={emailForm.body} onChange={(e) => setEmailForm({ ...emailForm, body: e.target.value })} placeholder="Email message" className="min-h-40 w-full rounded-2xl border border-slate-700 bg-black/30 p-4 text-white" /></div><Submit>Send Email and Save Copy</Submit></form></Modal> : null}
  </main>;
}
