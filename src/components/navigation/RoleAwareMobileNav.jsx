...
const ITEM_LIBRARY = {
  ...
  schedule: { label: "Schedule", icon: CalendarDays, routes: { CUSTOMER: "/calendar", SBO: "/calendar", EMPLOYEE: "/calendar", PM: "/pm/calendar" } },
...
function routeMatches(pathname, target) {
  ...
  if (["/customer", "/sbo", "/employee", "/pm"].includes(cleanTarget)) return pathname === cleanTarget;
...
function centerConfig(mode) {
  if (mode === "CUSTOMER") return { label: "SYNC", route: "/sync", sync: true };
...
function PreviewItem({ itemId }) {
  ...
  return <div className="sw-nav-settings-preview-item"><Icon aria-hidden="true" /><span>{item?.label || itemId}</span></div>;
}

export function MobileNavSettings() {
  const { user, mode } = useAuth();
  const [scope, setScope] = useState(["CUSTOMER", "SBO", "EMPLOYEE", "PM"].includes(mode) ? mode : "CUSTOMER");
  const [items, setItems] = useState(() => readPreference(scope, user));
  const [saved, setSaved] = useState("");

  useEffect(() => setItems(readPreference(scope, user)), [scope, user]);

  function changeSlot(index, nextValue) {
    setSaved("");
    setItems((current) => current.includes(nextValue) ? current : current.map((item, itemIndex) => itemIndex === index ? nextValue : item));
  }
  ...
  const center = centerConfig(scope);
  ...
  return (
    <section className="sw-nav-settings-card">
      ...
      <div className="sw-nav-settings-grid">
        {items.map((itemId, index) => <label key={`${scope}-${index}`}><span>Button {index + 1}</span><select value={itemId} onChange={(event) => changeSlot(index, event.target.value)}>{(ALLOWED[scope] || []).map((optionId) => <option key={optionId} value={optionId}>{ITEM_LIBRARY[optionId]?.label || optionId}</option>)}</select></label>)}
      </div>
      {saved ? <div className="sw-nav-settings-saved">{saved}</div> : null}
      <div className="sw-nav-settings-actions"><button type="button" onClick={reset}>Reset defaults</button><button type="button" className="primary" onClick={save}>Save navigation</button></div>
    </section>
  );
}
...
export default function RoleAwareMobileNav() {
  ...
  const hidden = useMemo(() => {
    ...
    return ["/login", "/register", "/upgrade", "/gamecast", "/sports/invite", "/social/invite"].some((prefix) => pathname.startsWith(prefix));
  }, [navMode, pathname]);

  useEffect(() => {
    if (!navMode) return undefined;
    const frameId = window.requestAnimationFrame(() => setItems(readPreference(navMode, user)));
    function handleChanged(event) {
      if (event?.detail?.mode === navMode) setItems(readPreference(navMode, user));
    }
  ...
  const renderedItems = items.map((itemId) => {
    ...
    return { id: itemId, label: item?.label || itemId, Icon: item?.icon || Home, target, active: routeMatches(pathname, target), badge: itemId === "inbox" ? unread : 0 };
  });

  function openCenter() {
    if (center.sync) {
      navigate(`/sync?return=${encodeURIComponent(location.pathname + location.search)}`);
      return;
    }
...
function NavButton({ item, onClick }) {
  ...
  return <button type="button" onClick={onClick} className={item.active ? "active" : ""} aria-current={item.active ? "page" : undefined}><span className="icon"><Icon aria-hidden="true" />{item.badge > 0 ? <i>{item.badge > 99 ? "99+" : item.badge}</i> : null}</span><span>{item.label}</span></button>;
}
