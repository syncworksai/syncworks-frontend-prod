...
export default function GlobalModeBar() {
  ...
  const linkClass = (active = false) => [
    ...
    active
      ...
      : "border-slate-800 bg-slate-950/70 text-slate-200 hover:border-cyan-500/25 hover:bg-cyan-500/8",
  ].join(" ");

  return (
    <div ref={rootRef} className="sticky top-0 z-[90] border-b border-cyan-400/15 bg-[#020617]/98 px-3 py-2 shadow-[0_10px_30px_rgba(0,0,0,0.38)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1900px] items-center justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-cyan-400/30 bg-cyan-400/10 px-3 text-sm font-black text-cyan-100"
          aria-label="Open SyncWorks app navigation"
          aria-expanded={open}
        >
          <MenuIcon open={open} />
          <span>Menu</span>
      ...
      {open ? (
        <>
          <button type="button" className="fixed inset-0 top-[3.55rem] z-[91] bg-black/70" aria-label="Close app navigation" onClick={() => setOpen(false)} />
          <div className="fixed left-3 right-3 top-[4.15rem] z-[92] max-h-[calc(100dvh-5rem)] overflow-y-auto rounded-[28px] border border-cyan-400/25 bg-[#050b16] p-4 shadow-[0_28px_90px_rgba(0,0,0,0.78)] sm:left-auto sm:w-[420px]">
            <div className="mb-3 flex items-center justify-between px-1">
              <div>
                <div className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-300">SyncWorks</div>
                <div className="mt-1 text-sm font-bold text-white">App navigation</div>
              </div>
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {modes.map(([key, item]) => {
                ...
                return (
                  <Link key={key} to={item.to} onClick={() => chooseMode(key)} className={linkClass(active)}>
                    <span>{item.label}</span>
                    {active ? <span className="text-[10px] uppercase tracking-[0.14em] text-cyan-300">Current</span> : null}
                  </Link>
                );
              })}
            </div>
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              {isPlatformAdmin ? (
                <Link to="/platform?tab=overview" onClick={() => setOpen(false)} className="flex min-h-12 items-center rounded-2xl border border-fuchsia-400/30 bg-fuchsia-400/10 px-4 text-sm font-black text-fuchsia-100">
