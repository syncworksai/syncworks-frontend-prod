...
import { AlertTriangle, Car, CloudRain, LocateFixed, Navigation, RefreshCw, ShoppingBag, Sparkles } from "lucide-react";
...
function tripStoreUrl(event) {
  const destination = [event?.location_name, event?.city, event?.state].filter(Boolean).join(", ");
  const params = new URLSearchParams({
    source: "calendar_trip",
    trip: String(event?.title || "Trip"),
    q: "trip essentials",
  });
  if (destination) params.set("destination", destination);
  return `/customer/store?${params.toString()}`;
}
  ...
  return (
    <section className="rounded-[1.9rem] border border-cyan-400/20 bg-[radial-gradient(circle_at_90%_0%,rgba(34,211,238,.13),transparent_32%),linear-gradient(145deg,rgba(15,23,42,.96),rgba(2,6,23,.96))] p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] text-cyan-200"><Sparkles className="h-3.5 w-3.5" />SYNC Assist · Travel + Weather</div><h2 className="mt-2 text-xl font-black text-white">Prepare for {event.title}</h2><p className="mt-1 text-sm text-slate-400">Traffic, weather, your arrival buffer and trip essentials become one prep plan.</p></div>
        <div className="flex flex-wrap gap-2">
          <a href={tripStoreUrl(event)} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-fuchsia-300/25 bg-fuchsia-500/10 px-4 text-xs font-black text-fuchsia-100"><ShoppingBag className="h-4 w-4" />Shop trip essentials</a>
      ...
      <div className="mt-3 rounded-2xl border border-fuchsia-300/15 bg-fuchsia-500/[.05] p-3 text-[10px] leading-5 text-slate-400"><span className="font-black text-fuchsia-100">Trip Store:</span> search luggage, chargers, rain gear, hydration, travel-size supplies or anything this trip needs. Approved affiliate links can plug into the same storefront path without changing Calendar.</div>
