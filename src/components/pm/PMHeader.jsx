...
export function PMNavigationMenu({ compact = false }) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <button type="button" onClick={() => setOpen((value) => !value)} className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-cyan-300/45 bg-[#07111f] text-cyan-50 shadow-[0_0_28px_rgba(34,211,238,0.24)]" aria-label="Open Property Management navigation" aria-expanded={open}>
        {compact ? <MenuIcon /> : <GearIcon />}
      </button>

      {open ? (
        <>
          <button type="button" className="fixed inset-0 z-[98] bg-black/70 md:hidden" aria-label="Close Property Management navigation" onClick={() => setOpen(false)} />
          <div className="fixed left-3 right-3 top-[8.9rem] z-[99] max-h-[calc(100dvh-10rem)] overflow-y-auto rounded-[28px] border border-cyan-300/35 bg-[#06101d] p-3 shadow-[0_24px_80px_rgba(0,0,0,0.75)] md:absolute md:left-auto md:right-0 md:top-12 md:w-80">
            <div className="mb-2 px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-cyan-300">Property Management</div>
            {items.map(([label, path], index) => (
              <button key={`${path}-${index}`} type="button" onClick={() => go(path)} className="flex min-h-12 w-full items-center rounded-2xl border border-transparent px-4 text-left text-sm font-semibold text-slate-100 transition hover:border-cyan-500/20 hover:bg-cyan-500/10">
                {label}
              </button>
            ))}
          </div>
...
export default function PMHeader({ title = "Property Management", subtitle = "", actions = null }) {
  ...
  return (
    <header className="sticky top-[57px] z-50 border-b border-cyan-500/20 bg-[#050914]/98 shadow-[0_12px_30px_rgba(0,0,0,0.45)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3">
        ...
        <button type="button" onClick={() => nav("/pm")} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-slate-700 bg-[#07111f] text-slate-100 md:hidden" aria-label="Go to Property Management home"><HomeIcon /></button>
