...
export function PMNavigationMenu({ compact = false }) {
  ...
  const items = [
    ["Dashboard", "/pm"],
    ["Add Property", "/pm/properties/new"],
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-cyan-500/25 bg-slate-950/90 text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,0.12)]"
        aria-label="Open Property Management menu"
        aria-expanded={open}
      >
        {compact ? <MenuIcon /> : <GearIcon />}
      </button>
      {open ? (
        <div className="absolute right-0 top-12 z-[90] w-60 overflow-hidden rounded-2xl border border-cyan-500/20 bg-[#07111f]/98 p-2 shadow-2xl backdrop-blur-xl">
          {items.map(([label, path]) => (
            <button
              key={path}
              type="button"
              onClick={() => { setOpen(false); nav(path); }}
              className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-semibold text-slate-200 transition hover:bg-cyan-500/10 hover:text-cyan-100"
            >
              {label}
            </button>
          ))}
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-cyan-500/20 bg-[#050914]/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3">
        <button type="button" onClick={() => nav("/pm")} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-cyan-500/25 bg-slate-950/80 shadow-[0_0_28px_rgba(34,211,238,0.14)]" aria-label="Open Property Management dashboard">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full border border-fuchsia-500/35 bg-fuchsia-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-fuchsia-200">Property Manager</span>
          </div>
        ...
        <button type="button" onClick={() => nav("/pm/settings")} className="hidden h-11 w-11 items-center justify-center rounded-2xl border border-slate-700 bg-slate-950/80 text-slate-100 md:inline-flex" aria-label="Property Management settings"><GearIcon /></button>
