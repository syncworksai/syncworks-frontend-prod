...
function MenuIcon() { return <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>; }
...
export function PMNavigationMenu({ compact = false }) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <button type="button" onClick={() => setOpen((value) => !value)} className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-cyan-500/25 bg-slate-950/90 text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,0.12)]" aria-label="Open Property Management menu" aria-expanded={open}>{compact ? <MenuIcon /> : <GearIcon />}</button>
      {open ? <div className="absolute right-0 top-12 z-[90] w-72 overflow-hidden rounded-2xl border border-cyan-500/20 bg-[#07111f]/98 p-2 shadow-2xl backdrop-blur-xl">{items.map(([label, path]) => <button key={path} type="button" onClick={() => { setOpen(false); nav(path); }} className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-semibold text-slate-200 transition hover:bg-cyan-500/10 hover:text-cyan-100">{label}</button>)}</div> : null}
    </div>
  );
}
