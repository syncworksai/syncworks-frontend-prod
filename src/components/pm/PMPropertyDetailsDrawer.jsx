...
const emptyProfile = { bedrooms: "", bathrooms: "", square_feet: "", year_built: "", furnished: false, utility_electric: "", utility_gas: "", utility_water: "", utility_trash: "", sewer_septic: "", hvac_details: "", roof_details: "", water_heater_details: "", access_details: "", insurance_details: "", warranty_notes: "", parking_details: "", safety_details: "", general_notes: "" };
...
export default function PMPropertyDetailsDrawer({ open, onClose, workspace, property, units = [], onChanged }) {
  ...
  async function load() {
    ...
    try {
      ...
      setProfile({ ...emptyProfile, ...(profileRes.data?.profile || {}) });
  ...
  return <div className="fixed inset-0 z-[260] bg-black/70" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="ml-auto flex h-full w-full max-w-3xl flex-col overflow-hidden border-l border-cyan-400/20 bg-[#040b14] shadow-2xl">
      <header className="flex items-start justify-between gap-4 border-b border-cyan-500/15 p-5"><div><div className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-300">Property Profile</div><h2 className="mt-2 text-2xl font-black text-white">Details & Inventory</h2><p className="mt-1 text-sm text-slate-400">{property?.name} · physical details and property-owned items stay here when tenants change.</p></div><button onClick={onClose} className="rounded-xl border border-slate-700 px-3 py-2 text-slate-300">✕</button></header>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        {!loading && section === "details" ? <div className="space-y-5">
          ...
          <div className="flex justify-end"><button disabled={saving} onClick={saveProfile} className="rounded-2xl bg-cyan-400 px-5 py-3 text-sm font-black text-slate-950">{saving ? "Saving..." : "Save Property Details"}</button></div>
        </div> : null}
        ...
        {!loading && section === "inventory" ? <div className="space-y-5">
          ...
          <section className="rounded-3xl border border-cyan-500/15 bg-[#07111f] p-4"><div className="flex items-center justify-between"><div><div className="text-xs font-black uppercase tracking-[.16em] text-cyan-300">{editing ? "Edit Item" : "Add Item"}</div><div className="mt-1 text-sm text-slate-400">Use room and unit fields for furnished or multi-unit inventory.</div></div>{editing ? <button onClick={() => { setEditing(null); setItem({ ...emptyItem, furnished_item: Boolean(profile.furnished) }); }} className="text-xs font-bold text-slate-400">Cancel edit</button> : null}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Item name"><input className={inputClass} value={item.name} onChange={(e) => setItem({ ...item, name: e.target.value })} placeholder="Example: Queen bed" /></Field><Field label="Category"><select className={inputClass} value={item.category} onChange={(e) => setItem({ ...item, category: e.target.value })}>{categories.map((value) => <option key={value} value={value}>{pretty(value)}</option>)}</select></Field><Field label="Room / location"><input className={inputClass} value={item.room_location} onChange={(e) => setItem({ ...item, room_location: e.target.value })} placeholder="Primary bedroom" /></Field><Field label="Unit"><select className={inputClass} value={item.unit || ""} onChange={(e) => setItem({ ...item, unit: e.target.value })}><option value="">Whole property</option>{units.map((unit) => <option key={unit.id} value={unit.id}>{unit.label}</option>)}</select></Field><Field label="Quantity"><input type="number" min="1" className={inputClass} value={item.quantity} onChange={(e) => setItem({ ...item, quantity: e.target.value })} /></Field><Field label="Condition"><select className={inputClass} value={item.condition} onChange={(e) => setItem({ ...item, condition: e.target.value })}>{conditions.map((value) => <option key={value} value={value}>{pretty(value)}</option>)}</select></Field><Field label="Brand"><input className={inputClass} value={item.brand} onChange={(e) => setItem({ ...item, brand: e.target.value })} /></Field><Field label="Model"><input className={inputClass} value={item.model_number} onChange={(e) => setItem({ ...item, model_number: e.target.value })} /></Field><Field label="Serial number"><input className={inputClass} value={item.serial_number} onChange={(e) => setItem({ ...item, serial_number: e.target.value })} /></Field><Field label="Replacement cost"><input inputMode="decimal" className={inputClass} value={item.replacement_cost ?? ""} onChange={(e) => setItem({ ...item, replacement_cost: e.target.value })} /></Field><Field label="Provider / servicer"><input className={inputClass} value={item.provider_name} onChange={(e) => setItem({ ...item, provider_name: e.target.value })} /></Field><Field label="Account / reference"><input className={inputClass} value={item.account_reference} onChange={(e) => setItem({ ...item, account_reference: e.target.value })} /></Field><Field label="Purchase date"><input type="date" className={inputClass} value={item.purchase_date || ""} onChange={(e) => setItem({ ...item, purchase_date: e.target.value })} /></Field><Field label="Warranty expiration"><input type="date" className={inputClass} value={item.warranty_expiration || ""} onChange={(e) => setItem({ ...item, warranty_expiration: e.target.value })} /></Field><label className="flex items-center gap-3 rounded-2xl border border-fuchsia-500/20 bg-fuchsia-500/5 p-3 text-sm font-bold text-fuchsia-100 sm:col-span-2"><input type="checkbox" checked={Boolean(item.furnished_item)} onChange={(e) => setItem({ ...item, furnished_item: e.target.checked })} />Include on furnished-unit / move-in inventory</label><Field label="Notes" span="sm:col-span-2"><textarea rows="3" className={`${inputClass} py-3`} value={item.notes} onChange={(e) => setItem({ ...item, notes: e.target.value })} /></Field></div><div className="mt-4 flex justify-end"><button disabled={saving} onClick={saveItem} className="rounded-2xl bg-cyan-400 px-5 py-3 text-sm font-black text-slate-950">{saving ? "Saving..." : editing ? "Save Changes" : "Add Item"}</button></div></section>

          <div className="space-y-3">{items.length ? items.map((row) => <article key={row.id} className="rounded-2xl border border-slate-800 bg-black/20 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><div className="font-black text-white">{row.quantity > 1 ? `${row.quantity} × ` : ""}{row.name}</div>{row.furnished_item ? <span className="rounded-full border border-fuchsia-400/25 px-2 py-1 text-[9px] font-black uppercase text-fuchsia-200">Furnished</span> : null}</div><div className="mt-1 text-xs text-slate-500">{[pretty(row.category), row.room_location, row.unit_label].filter(Boolean).join(" · ")}</div><div className="mt-2 text-xs text-slate-400">Condition: <span className="text-white">{pretty(row.condition)}</span>{row.brand ? ` · ${row.brand}${row.model_number ? ` ${row.model_number}` : ""}` : ""}{row.replacement_cost ? ` · ${money(row.replacement_cost)}` : ""}</div>{row.notes ? <div className="mt-2 text-xs text-slate-500">{row.notes}</div> : null}</div><div className="flex gap-2"><button onClick={() => beginEdit(row)} className="rounded-xl border border-cyan-400/30 px-3 py-2 text-xs font-black text-cyan-100">Edit Item</button><button onClick={() => deleteItem(row)} className="rounded-xl border border-rose-400/30 px-3 py-2 text-xs font-black text-rose-100">Delete Item</button></div></div></article>) : <div className="rounded-2xl border border-dashed border-slate-700 p-8 text-center text-sm text-slate-500">No property inventory items yet. Add furniture, appliances, keys, systems, or other property-owned items.</div>}</div>
        </div> : null}
      </div>
    </aside>
  </div>;
