...
import Section8CaseModal from "./Section8CaseModal";
...
function packetTone(pct, ready) {
  if (ready) return "emerald";
  if (pct >= 70) return "cyan";
  if (pct >= 40) return "amber";
  return "red";
}

function packetIcon(pct, ready) {
  if (ready || pct >= 100) return "✅";
  if (pct >= 70) return "🟦";
  if (pct >= 40) return "🟨";
  return "🟥";
}

export default function Section8CasesPanel({ onChanged }) {
  ...
  return (
    <>
      <Card
        ...
        subtitle="Simple view: Status • Inspection • Packet • Recert"
        right={
      ...
      >
        {err ? <div className="mb-3 text-sm text-red-300 bg-red-900/20 border border-red-800 rounded-2xl p-3">{err}</div> : null}

        <div className="grid lg:grid-cols-12 gap-3">
          <div className="lg:col-span-6">
            <div className="text-xs text-slate-400 mb-1">🔎 Search</div>
            <input className={inputCls()} value={q} onChange={(e) => setQ(e.target.value)} placeholder="voucher, tenant, property, unit, authority, email..." />
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="text-xs text-slate-400 mb-1">🧰 Inspection</div>
        ...
        <div className="mt-4 text-xs text-slate-400">{loading ? "Loading…" : `${rows.length} case${rows.length === 1 ? "" : "s"}`}</div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-950/60 text-slate-300">
              <tr className="text-left">
                ...
                <th className="p-3">📍 Property / Unit / Tenant</th>
                <th className="p-3 w-[190px]">👤 Agent</th>
                <th className="p-3 w-[140px]">🟢 Status</th>
                <th className="p-3 w-[170px]">🧰 Inspection</th>
                <th className="p-3 w-[140px]">📦 Packet</th>
                <th className="p-3 w-[150px]">📅 Recert</th>
            ...
            <tbody className="bg-slate-950/30">
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={9} className="p-5 text-slate-400">
                    No Section 8 cases yet.
                  </td>
                </tr>
              ) : (
                rows.map((c) => {
                  const pct = Number.isFinite(Number(c.packet_progress_pct)) ? Number(c.packet_progress_pct) : 0;
                  const ready = !!c.packet_ready;
                  const missing = Array.isArray(c.packet_missing_keys) ? c.packet_missing_keys : [];

                  return (
                    <tr
                      key={c.id}
                      className="border-t border-slate-800/70 hover:bg-slate-900/30 cursor-pointer"
                      onClick={() => openEdit(c)}
                      title="Click row to edit"
                    >
                      <td className="p-3" onClick={(e) => e.stopPropagation()}>
                        <div className="font-semibold text-slate-100">#{c.id}</div>
                        <div className="text-xs text-slate-400">{c.voucher_number || "—"}</div>
                      </td>

                      <td className="p-3">
                        <div className="text-slate-100 font-medium">{c.property_label || "—"}</div>
                        <div className="text-xs text-slate-400 mt-1">
                          <span className="text-slate-300">{c.unit_label || "—"}</span>
                          <span className="px-2 text-slate-600">•</span>
                          <span className="text-slate-300">{c.tenant_label || "—"}</span>
                        </div>
                        {c.housing_authority_name ? <div className="text-xs text-slate-500 mt-1">Authority: {c.housing_authority_name}</div> : null}
                      </td>

                      <td className="p-3">
                        <div className="text-slate-200">{c.caseworker_name || "—"}</div>
                        <div className="text-xs text-slate-500">{c.caseworker_email || ""}</div>
                      </td>
                      ...
                      <td className="p-3">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="text-xs text-slate-500">Sched: {formatDate(c.inspection_scheduled_date)}</div>
                        </div>
                      </td>

                      <td className="p-3">
                        <div className="flex flex-col gap-1">
                          <Pill tone={packetTone(pct, ready)}>
                            {packetIcon(pct, ready)} {pct || 0}%
                          </Pill>
                          {missing.length ? <div className="text-[11px] text-slate-500">Missing: {missing.length}</div> : <div className="text-[11px] text-emerald-200">Complete</div>}
                        </div>
                      </td>

                      <td className="p-3">
                        <div className="text-slate-200">{formatDate(c.recert_due_date)}</div>
                        <div className="text-xs text-slate-500">Subm: {formatDate(c.recert_submitted_date)}</div>
                      </td>

                      <td className="p-3">
                        <div className="text-slate-200">{c.updated_at ? new Date(c.updated_at).toLocaleString() : "—"}</div>
                      </td>
                      ...
                      <td className="p-3 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="inline-flex items-center gap-2">
                          ...
                          <Button tone="rose" onClick={() => remove(c)}>
                            🗑 Delete
                          </Button>
                        </div>
                      </td>
