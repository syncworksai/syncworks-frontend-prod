      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-[11px] leading-5 text-slate-400">
            {suggestion.reason || "Your last set suggests a small adjustment while keeping the exercise and plan intact."}
          </div>
        </div>
        <div className="shrink-0 rounded-xl border border-cyan-300/20 bg-black/25 px-3 py-2 text-right">
          ...
          <div className="mt-1 text-sm font-black text-cyan-100">
            {proposedWeight !== "" ? formatLoad(proposedWeight) : "Current load"}{proposedReps !== "" ? ` × ${proposedReps}` : ""}
          </div>
        </div>
      </div>
...
export default function WorkoutFocusCompactPanel({
  ...
  const workoutProgress = useMemo(() => {
    ...
    return {
      ...
      percent: exercises.length ? Math.round((resolved / exercises.length) * 100) : 0,
    };
  }, [exercises]);
  ...
  function updateTimer(patch) {
    ...
    try {
      window.localStorage.setItem(TIMER_PREFS_KEY, JSON.stringify(next));
    } catch {
      // Session preference remains in memory.
    }
  }
  ...
  return (
    <>
      <section className="overflow-hidden rounded-[1.7rem] border border-lime-300/25 bg-[radial-gradient(circle_at_88%_0%,rgba(57,255,136,0.11),transparent_28%),linear-gradient(160deg,#0a120d,#020403)] shadow-[0_22px_70px_rgba(0,0,0,0.46)]">
        <div className="border-b border-white/8 p-3 sm:p-4">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[9px] font-black uppercase tracking-[0.18em] text-lime-300">Active workout</div>
              <div className="mt-1 truncate text-base font-black text-white">{cleanText(session.workout_name, "Workout")}</div>
            </div>
          ...
          <div className="mt-2 flex items-center gap-2">
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-lime-300" style={{ width: `${workoutProgress.percent}%` }} /></div>
            <div className="text-[9px] font-black text-slate-500">{workoutProgress.resolved}/{workoutProgress.total}</div>
          </div>
        </div>
        ...
        <div className="p-3 pb-28 sm:p-4 sm:pb-32">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[9px] font-black uppercase tracking-[0.16em] text-cyan-200">Exercise {safeNumber(session.current_exercise_index, 0) + 1} of {Math.max(1, exercises.length)}</div>
          ...
          {image ? (
            <button type="button" onClick={() => setDrawerMode("insight")} className="mt-3 block h-44 w-full overflow-hidden rounded-2xl border border-white/10 bg-black/30 sm:h-64">
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            <div className="rounded-2xl border border-white/10 bg-black/25 p-3 text-center">
              <div className="text-[8px] font-black uppercase tracking-wider text-slate-500">Target reps</div>
              <div className="mt-1 text-lg font-black text-white">{reps}</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/25 p-3 text-center">
              <div className="text-[8px] font-black uppercase tracking-wider text-slate-500">Target load</div>
              <div className="mt-1 text-lg font-black text-white">{formatLoad(weight)}</div>
            </div>
            <button type="button" onClick={() => setDrawerMode("history")} className="rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.06] p-3 text-center">
              <div className="text-[8px] font-black uppercase tracking-wider text-cyan-200">Last time</div>
              <div className="mt-1 text-sm font-black text-cyan-100">{previousSet ? `${cleanText(previousSet.actual_reps || previousSet.reps, "—")} × ${formatLoad(previousSet.actual_weight || previousSet.weight)}` : "Baseline"}</div>
            </button>
          </div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            <button type="button" onClick={() => setDrawerMode("history")} className="h-11 rounded-xl border border-white/10 bg-white/[0.035] text-[10px] font-black text-slate-200">History</button>
            <button type="button" onClick={() => setDrawerMode("insight")} className="h-11 rounded-xl border border-white/10 bg-white/[0.035] text-[10px] font-black text-slate-200">Exercise Info</button>
            <button type="button" onClick={() => setDrawerMode("change")} disabled={session.set_active} className="h-11 rounded-xl border border-white/10 bg-white/[0.035] text-[10px] font-black text-slate-200 disabled:opacity-35">Swap / Skip</button>
          </div>

          {allSetsComplete ? (
            <button type="button" onClick={onEndWorkout} className="mt-3 h-14 w-full rounded-2xl border border-lime-300/40 bg-lime-300 text-sm font-black uppercase tracking-[0.08em] text-black">End Workout and Review</button>
          ) : null}
        </div>
      </section>
      ...
      <div className="fixed inset-x-0 bottom-0 z-[210] border-t border-lime-300/20 bg-[#020604]/96 px-3 pb-[calc(env(safe-area-inset-bottom)+0.65rem)] pt-2 backdrop-blur-xl">
        ...
        <div className="mx-auto mt-1 flex max-w-xl items-center justify-between px-1 text-[8px] font-black uppercase tracking-[0.13em] text-slate-600">
          <button type="button" onClick={onPause} className="py-1">{session.paused ? "Resume workout" : "Pause workout"}</button>
          <button type="button" onClick={() => setDrawerMode("timer")} className="py-1">Timers</button>
          <button type="button" onClick={() => setDrawerMode("more")} className="py-1">More</button>
        </div>
      </div>
      ...
      <WorkoutFocusDrawer
        ...
        onSkip={() => setDrawerMode("")}
        onComeBackLater={() => setDrawerMode("")}
