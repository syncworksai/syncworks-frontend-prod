...
export default function RequestDetailsCard({
  ...
  useEffect(() => {
    ...
    loadCustomerRequestProfile().then((profile) => {
      ...
      if (contactPreference === "either") {
        if (profile.preferredContactMethod === "SMS") setContactPreference("text");
        if (profile.preferredContactMethod === "EMAIL") setContactPreference("either");
      }
