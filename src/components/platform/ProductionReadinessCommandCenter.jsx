...
const STORAGE_KEY = "syncworks_production_readiness_external_v1";
...
function loadManualState() {
  const base = { external: initialState(EXTERNAL_GATES), certification: initialState(CERTIFICATION_FLOWS) };
  if (typeof window === "undefined") return base;
  try {
    const saved = JSON.parse(window.localStorage.getItem(STORAGE_KEY) || "{}");
    return {
      external: { ...base.external, ...(saved.external || {}) },
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[.02] p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {["PENDING", "PASSED", "BLOCKED", "WAIVED"].map((status) => <button key={status} type="button" onClick={() => onChange({ ...current, status, updated_at: new Date().toISOString() })} className={`rounded-xl border px-3 py-2 text-[11px] font-black transition ${current.status === status ? badge(status) : "border-white/10 bg-slate-950 text-slate-400 hover:text-white"}`}>{status}</button>)}
      </div>
      <input value={current.note || ""} onChange={(event) => onChange({ ...current, note: event.target.value, updated_at: new Date().toISOString() })} placeholder="Verification note / ticket / provider detail" className="mt-3 w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2.5 text-xs text-white outline-none placeholder:text-slate-600" />
    </div>
  );
}
...
export default function ProductionReadinessCommandCenter() {
  ...
  const [manual, setManual] = useState(() => loadManualState());
  ...
  useEffect(() => { load(); }, []);
  useEffect(() => { if (typeof window !== "undefined") window.localStorage.setItem(STORAGE_KEY, JSON.stringify(manual)); }, [manual]);
  ...
  function update(section, id, value) { setManual((current) => ({ ...current, [section]: { ...current[section], [id]: value } })); }
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="rounded-[1.8rem] border border-violet-400/15 bg-violet-500/[.035] p-5"><div className="text-[10px] font-black uppercase tracking-[.2em] text-violet-200">Provider / device verification</div><h3 className="mt-1 text-xl font-black text-white">External launch gates</h3><p className="mt-1 text-xs leading-5 text-slate-400">These cannot be honestly inferred from source code. God Mode records the actual provider/device signoff here.</p><div className="mt-4 grid gap-2 lg:grid-cols-2">{EXTERNAL_GATES.map((item) => <ManualGate key={item.id} item={item} value={manual.external[item.id]} onChange={(value) => update("external", item.id, value)} />)}</div></div>

      <div className="rounded-[1.8rem] border border-cyan-400/15 bg-cyan-500/[.03] p-5"><div className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-200">Production certification</div><h3 className="mt-1 text-xl font-black text-white">End-to-end workflow signoff</h3><p className="mt-1 text-xs leading-5 text-slate-400">Do not infer that one successful module proves another. Each major SyncWorks lifecycle gets its own production signoff.</p><div className="mt-4 grid gap-2 lg:grid-cols-2">{CERTIFICATION_FLOWS.map((item) => <ManualGate key={item.id} item={item} value={manual.certification[item.id]} onChange={(value) => update("certification", item.id, value)} />)}</div></div>
    </section>
  );
}
