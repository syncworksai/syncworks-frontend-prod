...
export default function ProductionReadinessCommandCenter() {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="rounded-[1.8rem] border border-cyan-400/15 bg-cyan-500/[.03] p-5"><div className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-200">Production certification</div><h3 className="mt-1 text-xl font-black text-white">End-to-end workflow signoff</h3><p className="mt-1 text-xs leading-5 text-slate-400">Do not infer that one successful module proves another. Each major SyncWorks lifecycle gets its own durable production signoff.</p><div className="mt-4 grid gap-2 lg:grid-cols-2">{CERTIFICATION_FLOWS.map((item) => <ManualGate key={item.id} item={manual.certification[item.id]} value={manual.certification[item.id]} onChange={(value) => update("certification", item.id, value)} />)}</div></div>
    </section>
  );
}
