  ...
  return (
    <section className="rounded-[1.75rem] border border-violet-400/15 bg-slate-950/55 p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <button type="button" onClick={() => nav("/settings?tab=CONNECTIONS")} className="rounded-2xl border border-violet-400/20 bg-violet-500/[.06] p-4 text-left"><div className="text-xs font-black uppercase text-violet-200">External email</div><div className="mt-1 font-black text-white">{external.available ? `${external.accounts?.length || 0} inbox connected` : "Turn email intelligence on"}</div><div className="mt-1 text-xs text-slate-500">Outlook available now · Gmail next</div></button>
      </div>
