...
export default function SyncAssistantMorningBrief({ onPlayBriefing, compact = false }) {
  ...
  const live = state.live || {};\n\n  if (compact) {\n    return (\n      <section className="rounded-[1.6rem] border border-cyan-400/15 bg-slate-950/55 p-4 sm:p-5">\n        <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-[9px] font-black uppercase tracking-[.18em] text-cyan-200">Needs your attention</div><h2 className="mt-1 text-lg font-black text-white">{attention.length ? `${attention.length} item${attention.length === 1 ? "" : "s"} may need you` : "You’re clear right now"}</h2></div><div className="flex gap-2"><button type="button" onClick={onPlayBriefing} className="rounded-xl border border-cyan-300/20 bg-cyan-500/[.07] px-3 py-2 text-[10px] font-black text-cyan-100">Hear brief</button><button type="button" onClick={() => nav("/sync")} className="rounded-xl border border-violet-300/20 bg-violet-500/[.07] px-3 py-2 text-[10px] font-black text-violet-100">Ask SYNC</button></div></div>\n        <div className="mt-3 grid gap-2 md:grid-cols-2">{attention.length ? attention.slice(0, 4).map((item, index) => <button key={`${item.category}-${index}`} type="button" onClick={() => item.action?.url && nav(item.action.url)} className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/20 p-3 text-left"><div><div className="text-sm font-black text-white">{item.title}</div><div className="mt-1 line-clamp-1 text-xs text-slate-500">{item.detail}</div></div>{item.action?.url ? <ExternalLink className="h-4 w-4 shrink-0 text-cyan-200" /> : null}</button>) : <div className="md:col-span-2 rounded-2xl border border-emerald-400/15 bg-emerald-500/[.05] p-3 text-xs font-bold text-emerald-100">No connected area currently needs immediate action.</div>}</div>\n      </section>\n    );\n  }
